...
function menuMovil(){
	if(mobileAndTabletcheck()) {//si Es es un dispositivo móvil
		$( "h2" ).remove();
		$.each($(".portfolio_item"),function(i,l){
			...
			if(w>360 && w<721)
				fs=22;
			$(l).append('<h3>&nbsp;</h3><a href="'+enlace+'"><h2 style="position:relative;width:100%;font-size:'+fs+'px">'+h3.innerHTML+'</h2></a>'
			);
		});
	}
...
$(document).ready(function() {
	if(document.getElementsByClassName("cerdo")[0]){
		var w = $(window).width();
		if(w<720)
			$('.cerdo').css({"marginTop":"0"});
		window.onresize=function(){
			var w = $(window).width();
			if(w<720)
				$('.cerdo').css({"marginTop":"0"});
			else
				$('.cerdo').css({"marginTop":"125px"});
		}
	}
    initOutdoor();
    initparallax();
	...
	if(document.getElementById("portfolio_horizontal_container")){
		setTimeout(function(){menuMovil()},3000);
		window.onresize=function(){
			menuMovil();
		}
	}
	if(document.getElementById("menu-bio")){
		var w = $(window).width();
		if(w<900){
			$(".h1bio").css({"fontSize":"22px","marginLeft":"-20px"});
			setTimeout(function(){$("#menu-bio-ul").hide(800);},2000);
			$("#menu-bio").css({"left":"0px"});
			$("section").css({"padding":"20px"});
			$("#menu-bio-tog").show();
			$("#menu-bio-tog").click(function(){$("#menu-bio-ul").toggle()});
			// $("#menu-bio").show();
		}else{
			$("#menu-bio-tog").hide();
		}
